...
def run_difference_calculation(model_folder="t5-large", model_name="t5", model_size="large", function_type="l1"):
    ...
    org_model, org_config = get_models(f'{model_name}-{model_size}')

    org_dict = org_model.state_dict()
    trained_dict = model.state_dict()

    if "encoder" in MODELS_TO_LAYERS[model_name]:
        table_file_encoder = open(f'{function_type}_encoder_{model_name}.tsv', 'w')
        results_encoder = defaultdict(list)
        for encoder_n in range(MODELS_TO_LAYERS[model_name][model_size]):
    ...
    if "decoder" in MODELS_TO_LAYERS[model_name]:
        table_file_decoder = open(f'{function_type}_decoder_{model_name}.tsv', 'w')
        results_decoder = defaultdict(list)
        for decoder_n in range(MODELS_TO_LAYERS[model_name][model_size]):
            for dict_key in MODELS_TO_STORE[model_name][encoder]:
                dict_names = MODELS_TO_STORE[model_name][encoder]
...
if __name__ == "__main__":
    ...
    run_difference_calculation(args.model_folder, args.model_name, args.model_size, args.function_type)